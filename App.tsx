...
const App: React.FC = () => {
  ...
  return (
    // Added h-screen-safe and safe area paddings
    <div className={`flex flex-col md:flex-row h-screen-safe overflow-hidden transition-colors duration-300 pt-safe pb-safe ${config.theme === 'light' ? 'bg-slate-100 text-slate-900' : 'bg-background text-text'}`}>
        ...
        <div className="flex flex-col h-full overflow-y-auto custom-scrollbar">
            ...
            <div className="p-4 mt-auto space-y-3">
                ...
                <div className={`p-3 rounded-xl border text-xs text-muted flex items-center ${config.theme === 'light' ? 'bg-slate-50 border-slate-200' : 'bg-slate-900/50 border-slate-700'} ${!isSidebarOpen ? 'justify-center' : 'gap-2'}`}>
                    ...
                    {isSidebarOpen && (
                         <span className="font-semibold text-success truncate">Connected OKX V5</span>
                    )}
                </div>
            </div>
        </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full overflow-hidden bg-transparent relative">
        <div className="flex-1 overflow-y-auto p-0 md:p-6 custom-scrollbar">
            ...
            {currentView === 'dashboard' && <div className="p-4 md:p-0 flex-1"><Dashboard balances={balances} service={service} t={t} theme={config.theme} onAction={addToast} refreshInterval={config.refreshInterval} /></div>}
            {currentView === 'trade' && <div className="h-full"><TradeInterface service={service} onPlaceOrder={addToast} t={t} theme={config.theme} refreshInterval={config.refreshInterval} colorMode={config.colorMode} /></div>}
            {currentView === 'history' && <div className="p-4 md:p-0 flex-1"><HistoryAnalysis service={service} t={t} /></div>}
            {currentView === 'settings' && <div className="p-4 md:p-0 flex-1"><Settings config={config} onSave={(newConfig) => {
                setConfig(newConfig);
